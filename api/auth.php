...
$input=json_decode(file_get_contents('php://input'),true)?:[];$action=$input['action']??'register';
...
if($action==='register'){$email=strtolower(trim((string)($input['email']??'')));$password=(string)($input['password']??'');$name=trim((string)($input['name']??''));$company=trim((string)($input['company']??''));$role=in_array(($input['role']??'buyer'),['buyer','seller'],true)?$input['role']:'buyer';if(!filter_var($email,FILTER_VALIDATE_EMAIL)||strlen($password)<10){http_response_code(422);echo json_encode(['ok'=>false,'error'=>'Use a valid email and a password of at least 10 characters.']);exit;}try{$s=$pdo->prepare('INSERT INTO users(email,password_hash,role,name,company) VALUES(?,?,?,?,?)');$s->execute([$email,password_hash($password,PASSWORD_DEFAULT),$role,$name?:null,$company?:null]);echo json_encode(['ok'=>true,'user_id'=>(int)$pdo->lastInsertId(),'message'=>'Account created.']);}catch(PDOException $e){http_response_code(409);echo json_encode(['ok'=>false,'error'=>'An account with this email may already exist.']);}exit;}
if($action==='login'){$email=strtolower(trim((string)($input['email']??'')));$password=(string)($input['password']??'');$s=$pdo->prepare('SELECT id,email,password_hash,role,name,company FROM users WHERE email=? LIMIT 1');$s->execute([$email]);$u=$s->fetch();if(!$u||!password_verify($password,$u['password_hash'])){http_response_code(401);echo json_encode(['ok'=>false,'error'=>'Invalid email or password.']);exit;}session_regenerate_id(true);$_SESSION['user']=['id'=>(int)$u['id'],'email'=>$u['email'],'role'=>$u['role'],'name'=>$u['name'],'company'=>$u['company']];echo json_encode(['ok'=>true,'user'=>$_SESSION['user']]);exit;}
