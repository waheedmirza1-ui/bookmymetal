...
$video = trim((string)($in['video_url'] ?? ''));
...
$s->execute([$seller,$title,$description ?: null,$category,$video ?: null,$video ? 'review' : 'draft']);
echo json_encode(['ok'=>true,'product_id'=>(int)$pdo->lastInsertId(),'video_status'=>$video ? 'review' : 'draft']);
