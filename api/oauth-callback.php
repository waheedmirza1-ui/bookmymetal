...
try {
    ...
    $find = $pdo->prepare('SELECT id,email,role,name,company,role_confirmed,seller_enabled,oauth_provider,oauth_subject FROM users WHERE oauth_provider=? AND oauth_subject=? LIMIT 1'); $find->execute([$provider,$subject]); $user = $find->fetch();
    if (!$user) { $byEmail = $pdo->prepare('SELECT id,email,role,name,company,role_confirmed,seller_enabled,oauth_provider,oauth_subject FROM users WHERE email=? LIMIT 1'); $byEmail->execute([$email]); $user = $byEmail->fetch(); if ($user && $user['oauth_provider'] && ($user['oauth_provider'] !== $provider || $user['oauth_subject'] !== $subject)) throw new RuntimeException('This email is already linked to another sign-in method.'); if ($user) { $link = $pdo->prepare('UPDATE users SET oauth_provider=?,oauth_subject=?,email_verified=1,role_confirmed=1,name=COALESCE(NULLIF(name,\'\'),?) WHERE id=?'); $link->execute([$provider,$subject,$name,$user['id']]); } else { $create = $pdo->prepare("INSERT INTO users(email,password_hash,role,name,email_verified,role_confirmed,seller_enabled,oauth_provider,oauth_subject) VALUES(?,?, 'buyer',?,1,1,0,?,?)"); $create->execute([$email,password_hash(bin2hex(random_bytes(32)),PASSWORD_DEFAULT),$name?:null,$provider,$subject]); } $byEmail->execute([$email]); $user=$byEmail->fetch(); }
    session_regenerate_id(true); $_SESSION['user']=['id'=>(int)$user['id'],'email'=>$user['email'],'role'=>$user['role'],'seller_enabled'=>user_can_sell($user),'name'=>$user['name'],'company'=>$user['company'],'role_confirmed'=>true];
    oauth_redirect($returnPath);
