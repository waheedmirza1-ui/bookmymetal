...
try {
    ...
    if ($action === 'verify') {
        ...
        $field=$normalized['channel']==='email'?'email':'phone';$userStatement=$pdo->prepare("SELECT id,email,phone,role,name,company,role_confirmed,seller_enabled FROM users WHERE {$field}=? LIMIT 1");$userStatement->execute([$normalized['destination']]);$user=$userStatement->fetch();
        if($user){session_regenerate_id(true);$_SESSION['user']=['id'=>(int)$user['id'],'email'=>$user['email'],'phone'=>$user['phone'],'role'=>$user['role'],'seller_enabled'=>user_can_sell($user),'name'=>$user['name'],'company'=>$user['company'],'role_confirmed'=>true];otp_json(['ok'=>true,'next'=>'signed_in','user'=>$_SESSION['user']]);}
    ...
    if ($action === 'complete_profile') {
        ...
        $name=trim((string)($input['name'] ?? ''));$company=trim((string)($input['company'] ?? ''));if(strlen($name)<2) otp_json(['ok'=>false,'error'=>'Enter your name.'],422);
        $email=$pending['channel']==='email'?$pending['destination']:null;$phone=$pending['channel']==='sms'?$pending['destination']:null;$create=$pdo->prepare("INSERT INTO users(email,phone,password_hash,role,name,company,email_verified,phone_verified,role_confirmed,seller_enabled) VALUES(?,?,?,'buyer',?,?,?, ?,1,0)");$create->execute([$email,$phone,password_hash(bin2hex(random_bytes(32)),PASSWORD_DEFAULT),$name,$company?:null,$email?1:0,$phone?1:0]);$id=(int)$pdo->lastInsertId();unset($_SESSION['otp_signup']);session_regenerate_id(true);$_SESSION['user']=['id'=>$id,'email'=>$email,'phone'=>$phone,'role'=>'buyer','seller_enabled'=>false,'name'=>$name,'company'=>$company?:null,'role_confirmed'=>true];otp_json(['ok'=>true,'user'=>$_SESSION['user']]);
    }
